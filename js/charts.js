const RescueCharts = {
  ...
  thresholdPlugin: {
    ...
    beforeDatasetsDraw(chart) {
      ...
      const thresholds = RescueData?.thresholds || {};

      const dangerMax =
        Number(thresholds.danger?.max) || 20;

      const warningMax =
        Number(thresholds.warning?.max) || 50;

      const chartMax =
        Number(scales.y.max) || 120;
      ...
      zones.forEach(zone => {
        const from =
          Math.max(0, Math.min(chartMax, zone.from));

        const to =
          Math.max(0, Math.min(chartMax, zone.to));
        ...
        const yTop =
          scales.y.getPixelForValue(to);

        const yBottom =
          scales.y.getPixelForValue(from);
